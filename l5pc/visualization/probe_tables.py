"""Visualization: 3-level Ridge ΔR² results tables."""
import json
import numpy as np
...
def format_results_table(results, level_name, hidden_size):
    ...
    sep = "=" * 90
    col_header = f"{'Variable':<30} | {'R²_trained':>10} | {'R²_untrained':>12} | {'ΔR²':>8} | {'Category':<20}"
    col_sep = "-" * 90

    lines = [sep, header, sep, col_header, col_sep]

    # Sort by ΔR² descending
    ...
    lines.append(f"Summary: {n_learned}/{n_total} non-zombie (ΔR² > 0.1), {n_strong} strong (ΔR² > 0.2)")
    lines.append("")
...
def plot_delta_r2_bar(results, level_name, hidden_size, save_path=None):
    """Bar chart of ΔR² values for one level."""
    if not results:
        return
    ...
    # Threshold lines
    ax.axhline(y=0.1, color='orange', linestyle='--', alpha=0.7, label='ΔR² = 0.1 (learned)')
    ax.axhline(y=0.2, color='green', linestyle='--', alpha=0.7, label='ΔR² = 0.2 (strong)')

    ax.set_xticks(range(len(names)))
    ax.set_xticklabels(names, rotation=45, ha='right', fontsize=8)
    ax.set_ylabel('ΔR² (trained − untrained)')
    ax.set_title(f'Level {level_name} Ridge ΔR² — LSTM h={hidden_size}')
...
def plot_cross_hidden_comparison(results_dir, level='B', save_path=None):
    """Compare ΔR² across hidden sizes for one level."""
    results_dir = Path(results_dir)
    ...
    ax.set_ylabel('ΔR²')
    ax.set_title(f'Level {level} ΔR² across hidden sizes')
    ax.legend()
    plt.tight_layout()
